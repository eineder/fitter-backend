...
describe("Given two authenticated users, use A and user B", () => {
  ...
  describe("When user A sends a tweet", () => {
    ...
    describe("When user B replies to user A's tweet", () => {
      ...
      let userBtweetsCount, userBtimelineLength;
      beforeAll(async () => {
        userBtweetsCount = (await dynamoUtil.getUser(userB.username)).Item
          .tweetsCount;
        userBtimelineLength = (await dynamoUtil.getTimeline(userB.username))
          .Items.length;
